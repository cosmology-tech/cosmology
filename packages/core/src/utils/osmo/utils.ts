 ...
 * @returns {CoinSymbol}
 */
export const getSymbolForCoinGeckoId = (
  geckoId: CoinGeckoToken
): CoinSymbol => {
...
export const symbolsAndDisplayValuesToCoinsArray = (
  coins: DisplayCoin[]
): Coin[] =>
...
export const convertValidatorPricesToDenomPriceHash = (
...
export const getPoolByGammName = (
  pools: PoolDisplay[],
  gammId: string
): PoolDisplay => {
...
export const calculateShareOutAmount = (
  poolInfo: Pool,
  coinsNeeded: Coin[]
) => {
  const shareOuts = []
...
export const calculateCoinsNeededInPoolForValue = (
  prices: PriceHash,
  poolInfo: PoolPretty,
  value
) => {
...
export const makeLcdPoolPretty = (
  ...
  const liquidity = tokens
    .reduce((m, v) => {
      return m.add(new Dec(v.value))
    }, new Dec('0'))
    .toString()

  const nickname = tokens
    .reduce((m, v) => {
      return [...m, v.symbol]
    }, [])
    .join('/')
