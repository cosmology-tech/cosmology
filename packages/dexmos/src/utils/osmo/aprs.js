...
export const getPoolAprs = async ({ api, validator, poolIds, liquidityLimit = 100_000, lockup = "14" }) => {
    ...
    const { lockable_durations: [d1, d7, d14] } = await api.getLockableDurations();
    ...
    const incentives = incentivized_pools.filter(({ pool_id, lockable_duration }) => poolIds.includes(pool_id) && lockable_duration === ldur);
